...
type InvitationForm = { recipientEmail: string; recipientType: Invitation['recipientType']; organizationId: string; clinicId: string; expiresAt: string }
const emptyForm: InvitationForm = { recipientEmail: '', recipientType: 'doctor', organizationId: '', clinicId: '', expiresAt: '' }
...
export function InvitationsPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!/^\S+@\S+\.\S+$/.test(form.recipientEmail.trim())) { setFormError('يرجى إدخال بريد إلكتروني صحيح.'); return }
    ...
    try {
      await addRecord('invitations', { recipientEmail: form.recipientEmail.trim().toLowerCase(), recipientType: form.recipientType, organizationId: form.organizationId, clinicId: form.clinicId, status: 'pending', expiresAt: Timestamp.fromDate(new Date(`${form.expiresAt}T23:59:59`)) }, user.uid)
